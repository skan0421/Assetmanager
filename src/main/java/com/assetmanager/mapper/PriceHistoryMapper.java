...
public interface PriceHistoryMapper {
     ...
     * 가격 히스토리 등록
     */
    @Insert("INSERT INTO price_history (symbol, exchange, price, volume, high_price, low_price, open_price, close_price, change_rate, price_timestamp, created_at) " +
            "VALUES (#{symbol}, #{exchange}, #{price}, #{volume}, #{highPrice}, #{lowPrice}, #{openPrice}, #{closePrice}, #{changeRate}, #{timestamp}, NOW())")
     ...
     * ID로 가격 히스토리 조회
     */
    @Select("SELECT id, symbol, exchange, price, volume, high_price, low_price, open_price, close_price, change_rate, price_timestamp AS timestamp " +
     ...
     * 가격 정보 수정
     */
    @Update("UPDATE price_history SET price = #{price}, volume = #{volume}, high_price = #{highPrice}, low_price = #{lowPrice}, " +
            "open_price = #{openPrice}, close_price = #{closePrice}, change_rate = #{changeRate}, price_timestamp = #{timestamp} WHERE id = #{id}")
    ...
    @Insert({
        "<script>",
        "INSERT INTO price_history (symbol, exchange, price, volume, high_price, low_price, open_price, close_price, change_rate, price_timestamp, created_at)",
        "VALUES",
        "<foreach collection='list' item='item' separator=','>",
        "(#{item.symbol}, #{item.exchange}, #{item.price}, #{item.volume}, #{item.highPrice}, #{item.lowPrice}, #{item.openPrice}, #{item.closePrice}, #{item.changeRate}, #{item.timestamp}, NOW())",
        "</foreach>",
        "</script>"
    })
    ...
    @Insert("INSERT INTO price_history (symbol, exchange, price, volume, high_price, low_price, open_price, close_price, change_rate, price_timestamp, created_at) " +
            "VALUES (#{symbol}, #{exchange}, #{price}, #{volume}, #{highPrice}, #{lowPrice}, #{openPrice}, #{closePrice}, #{changeRate}, #{timestamp}, NOW()) " +
            "ON DUPLICATE KEY UPDATE price = #{price}, volume = #{volume}, high_price = #{highPrice}, low_price = #{lowPrice}, open_price = #{openPrice}, close_price = #{closePrice}, change_rate = #{changeRate}, price_timestamp = #{timestamp}")
    ...
    @Select("SELECT COUNT(*) FROM price_history WHERE symbol = #{symbol} AND exchange = #{exchange} AND price_timestamp = #{timestamp}")
